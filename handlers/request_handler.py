...
class HTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if raw_json:
            ...
            if self.path in ("/resources/id", "/resources/id/"):
                ...
                resources = str(
                    self.db.get_resources_by_type(resource_type=data["type"])
                )
    ...
    def do_POST(self):
        ...
        content_len = int(self.headers.get("content-length", 0))
        post_body = self.rfile.read(content_len)
        root_logger.info(post_body)
        raw_json = post_body.decode("UTF-8")
        if self.path in ("/resource_types", "/resource_types/"):
            if raw_json:
                data = json.loads(raw_json)
                self.db.create_resource_type(
                    resource_name=data["type"], max_speed=data["max_speed"]
                )
